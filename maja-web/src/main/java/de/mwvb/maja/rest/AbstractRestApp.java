...
public abstract class AbstractRestApp extends AbstractWebApp {
	// TODO Vermutlich muss man mehr Verantwortung an AbstractRestService geben. AbstractRestService könnte einen Standard-Builder liefern, der
	...
	protected <T> void addRestService(Class<? extends AbstractNoEntityRestService> restServiceClass) {
		...
		if (n == 0) {
			throw new RuntimeException("REST service class " + restServiceClass.getName() + " has no @" + Path.class.getSimpleName() + " annotated methods");
		}
	}
	...
	protected <T> Route createRoute(Class<? extends AbstractNoEntityRestService> restServiceClass, Method method) {
		...
		switch (args) {
			...
			case 1: // typically methods for verbs PUT or POST
				...
				break;
			default: // >1
				throw new RuntimeException("Unsupported number of method parameters: " + method.getName() + "(" + args + ")");
		}
		return route;
	}
}
